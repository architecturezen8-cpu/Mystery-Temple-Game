/* ═══════════════════════════════════════════════════════════════════════════
   ...
   FIXED VERSION:
   - All UI buttons respond instantly (no 300ms delay)
   - Touch events properly handled
   - Visual feedback on all interactions
   - FPS limiting integrated
   ═══════════════════════════════════════════════════════════════════════════ */
...
/* ╔═══════════════════════════════════════════════════════════════════════════╗
   ║  UPDATE LOOP (with FPS limiting & mobile camera)                          ║
   ╚═══════════════════════════════════════════════════════════════════════════╝ */
...
function update(timestamp) {
    ...
    if (player) {
        if (window.isMobile || window.DeviceProfiler?.detect().isMobile) {
            // Smoothly move camera horizontally to follow player
            let targetCamX = player.position.x * 0.4;
...
function animate(timestamp) {
    ...
    // ═══════════════════════════════════════════════════════════════════
    // FPS LIMITING - Check if we should render this frame
    // ═══════════════════════════════════════════════════════════════════
    if (window.FPSManager) {
        const timing = window.FPSManager.checkFrameTiming(timestamp);
        if (!timing.shouldRender) {
            return; // Skip this frame - too soon
        }
        // Update deltaTime from FPS manager if available
        if (timing.deltaTime) {
...
/* ╔═══════════════════════════════════════════════════════════════════════════╗
   ║  INSTANT BUTTON BINDINGS - Remove 300ms delay from all UI buttons         ║
   ╚═══════════════════════════════════════════════════════════════════════════╝ */

function bindAllUIButtons() {
    // Use the fastBindBtn from 12-controls.js if available
    const bindFn = window.fastBindBtn || window.bindUIButton;

    if (!bindFn) {
        console.warn('fastBindBtn not available, buttons may have delay');
        return;
    }

    // Map of button IDs to their functions
    const buttonBindings = {
        ...
        // Any close buttons
        'closeBtn': () => {
            // Generic close - hide all overlays
    ...
    // Also bind any elements with onclick attributes
    document.querySelectorAll('[onclick]').forEach(el => {
        ...
        if (typeof fn === 'function') {
            ...
            // Bind with fast handler
            bindFn(el, fn);
        }
    });

    console.log('✅ All UI buttons bound with instant response');
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Init controls (from 12-controls.js)
    if (typeof initControls === 'function') {
        initControls();
    }

    // Init camera privacy translate (if exists)
    ...
    // Init optimizations (from 14-optimization.js)
    if (typeof initOptimizations === 'function') {
        initOptimizations();
    }

    // ═══════════════════════════════════════════════════════════════════
    // BIND ALL UI BUTTONS WITH INSTANT RESPONSE
    // ═══════════════════════════════════════════════════════════════════
    // Wait a bit for fastBindBtn to be available from 12-controls.js
...
console.log('✅ 13-main.js loaded (instant UI response + FPS limiting)');