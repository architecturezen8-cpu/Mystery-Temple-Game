...
(function applyUltraLowEndFixes() {
    ...
    console.log('📱 Device detected:', { isMobile: window.isMobile, isLow, isUltraLow, mem, cpu, w });
...
(function() {
    ...
    const cleanupInterval = window.isMobile ? 10000 : 15000;
    
    setInterval(() => {
        if (!gameRunning) return;

        const particleLimit = window.isMobile ? 20 : 35;
        if (typeof particles !== 'undefined' && particles.length > particleLimit) {
            const removeCount = window.isMobile ? 15 : 10;
        ...
        const trailLimit = window.isMobile ? 15 : 25;
        if (typeof magicTrails !== 'undefined' && magicTrails.length > trailLimit) {
            const removeCount = window.isMobile ? 10 : 5;
        ...
        const burstLimit = window.isMobile ? 8 : 12;
        if (typeof gemBursts !== 'undefined' && gemBursts.length > burstLimit) {
            const removeCount = window.isMobile ? 5 : 4;
    ...
    if (typeof QUALITY !== 'undefined') {
        const reduction = window.isMobile ? 0.7 : 0.85;
        ...
        QUALITY.maxGems = Math.min(QUALITY.maxGems, window.isMobile ? 4 : 6);
        QUALITY.maxObstacles = Math.min(QUALITY.maxObstacles, window.isMobile ? 4 : 6);
        QUALITY.maxGreenGems = Math.min(QUALITY.maxGreenGems, 2);
        QUALITY.maxRedGems = Math.min(QUALITY.maxRedGems, 1);
        QUALITY.maxBoosts = 1;
        QUALITY.shootingStarInterval = window.isMobile ? 10000 : 6000;
    ...
    const waitForRenderer = setInterval(() => {
        if (typeof renderer !== 'undefined' && renderer) {
            ...
            const maxPR = window.isMobile ? 1 : (window.__PIXEL_RATIO_MAX__ || 1.5);
...
(function optimizeGameLoop() {
    ...
    window.animate = function() {
        ...
        if (window.isMobile || /Android|iPhone|iPad|iPod/i.test(navigator.userAgent)) {
